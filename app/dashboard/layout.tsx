...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div
      className={`${spaceGrotesk.variable} font-sans min-h-screen bg-[#f2f2f2] text-gray-800 overflow-x-hidden`}
    >
      {/* Sidebar - responsive for mobile with translucent effect */}
      <div
        className={`fixed h-full bg-white/70 backdrop-blur-xl border-r border-gray-200/50 shadow-lg transition-all duration-300 z-50 ${sidebarCollapsed ? "-translate-x-full md:translate-x-0 md:w-20" : "translate-x-0 w-64"}`}
      >
      ...
      {/* Main Content */}
      <div
        className={`transition-all duration-300 ${sidebarCollapsed ? "md:ml-20" : "md:ml-64"} ml-0 overflow-x-hidden`}
      >
        {children}
      </div>
    </div>
