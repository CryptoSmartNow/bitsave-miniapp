    ...
    <div className="flex-shrink-0 w-[320px] sm:w-[380px] md:w-[420px] lg:w-[450px] bg-white/80 backdrop-blur-xl rounded-2xl border border-white/60 shadow-lg relative overflow-hidden">
      ...
      <div className="relative z-10 p-4">
        {tweetId ? (
          <div
            ref={containerRef}
            className="tweet-container min-h-[150px] max-h-[400px] w-full overflow-hidden"
          >
            {!isLoaded && !error && (
                ...
                <div className="mb-4">
                  ...
                  {loadingTimeout ? (
                    <p className="text-gray-500 mb-4 text-sm">
                      This tweet couldn&apos;t be loaded due to browser security settings or ad
                      blockers.
                    </p>
                  ) : (
                    <p className="text-gray-500 mb-4 text-sm">
          ...
          <div className="p-6 text-center">
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="text-[#81D7B4] hover:underline"
            >
              View on X
            </a>
          </div>
...
const TwitterFeed = ({ links }: { links: string[] }) => {
  ...
  useEffect(() => {
    ...
    script.onerror = () => {
      console.warn(
        "Twitter widgets script failed to load - likely blocked by ad blocker or network restrictions",
      );
      setScriptError(true);
    };
    ...
    >
      <div className="flex w-max gap-6 pb-4">
        {duplicatedLinks.map((url, index) => {
          ...
          return <TwitterCard key={`${tweetId}-${keyPrefix}`} url={url} index={index} />;
        })}
      </div>
    </div>
  );
