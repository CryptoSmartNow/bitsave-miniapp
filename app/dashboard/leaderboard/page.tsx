...
export default function LeaderboardPage() {
    ...
    const fetchLeaderboardData = async () => {
      ...
      try {
        ...
        if (currentUserAddress) {
          const userPosition = rankedData.find(
            (user) => user.useraddress.toLowerCase() === currentUserAddress.toLowerCase(),
          );
  ...
  const UserIcon = ({ rank }: { rank: number }) => {
    ...
    return (
      <div
        className={`w-10 h-10 rounded-full ${bgColor} flex items-center justify-center ${textColor} border-2 border-white shadow-md`}
      >
        <svg
  ...
  return (
    <div
      className={`${spaceGrotesk.className} p-4 sm:p-6 md:p-8 bg-[#f2f2f2] text-gray-800 relative min-h-screen pb-8 overflow-x-hidden`}
    >
      {/* Grain overlay */}
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 md:mb-8 relative z-10">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-gray-800 tracking-tight mb-2">
            Leaderboard
          </h1>
      ...
      {leaderboardData.length > 0 && (
        <div className="hidden md:flex justify-center items-end mb-12 relative z-10">
          {leaderboardData.slice(0, Math.min(3, leaderboardData.length)).map((user, index) => {
            ...
            const podiumHeight =
              position === 0
                ? "h-36" // 1st place (tallest)
                : position === 1
                  ? "h-28" // 2nd place (medium)
            ...
            return (
              ...
              >
                <div className="flex flex-col items-center">
                  ...
                  <div
                    className={`${podiumHeight} w-28 bg-gradient-to-t from-white/90 to-white/70 backdrop-blur-md rounded-t-lg border border-white/60 shadow-[0_8px_20px_-6px_rgba(0,0,0,0.2)] flex items-center justify-center`}
                  >
                    <span className="font-bold text-xl text-gray-800">
                      {user2.rank === 1 ? "🏆" : user2.rank === 2 ? "🥈" : "🥉"}
                    </span>
                  </div>
                </div>
              </motion.div>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between">
          ...
          <div className="flex items-center">
            ...
            <div className="bg-white/80 backdrop-blur-sm rounded-xl p-3 border border-white/60 shadow-sm">
              ...
              <p className="font-bold text-2xl text-gray-800">
                {currentUserPosition ? `$${currentUserPosition.totalamount.toFixed(2)}` : "$0.00"}
              </p>
            </div>
          </div>
        </div>
              ...
              <div className="p-6">
                <div className="grid grid-cols-2 gap-4 mb-6">
                  ...
                  <div className="bg-white/50 backdrop-blur-sm rounded-xl p-4 border border-gray-100">
                    ...
                    <div className="flex items-center">
                      <img
                        src={`/${selectedUser.chain.toLowerCase()}.svg`}
        ...
        <div className="space-y-4">
          <div className="bg-white/50 backdrop-blur-sm rounded-xl p-4 border border-gray-100">
            ...
            <p className="text-sm text-gray-600">
              The leaderboard ranks users based on their total savings amount across all plans. The
              more you save, the higher your rank!
            </p>
          </div>
          ...
          <div className="bg-white/50 backdrop-blur-sm rounded-xl p-4 border border-gray-100">
            ...
            <p className="text-sm text-gray-600">
              The leaderboard is updated in real-time as users make deposits and withdrawals to
              their savings plans.
            </p>
          </div>
        </div>
      ...
      <div className="mt-12 mb-8 text-center">
        ...
        <p className="mt-3 text-sm text-gray-500">
          Create a new savings plan and start climbing the leaderboard today!
        </p>
      </div>
    </div>
  );
