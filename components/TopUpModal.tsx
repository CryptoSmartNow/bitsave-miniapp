...
export default function TopUpModal({
      ...
      if (amountNum > 0) {
        if (isEth) {
          ...
          if (nativeBalanceNum < totalNeeded) {
            setBalanceWarning(
              `Insufficient ETH balance. You have ${nativeBalanceNum.toFixed(6)} ETH but need ${totalNeeded.toFixed(6)} ETH (including gas fees).`,
            );
          }
        } else {
          // For token top-ups
          if (tokenBalanceNum < amountNum) {
            setBalanceWarning(
              `Insufficient ${tokenName} balance. You have ${tokenBalanceNum.toFixed(4)} ${tokenName} but need ${amountNum} ${tokenName}.`,
            );
  ...
  const handleStablecoinTopUp = async (amount: string, savingsPlanName: string) => {
    ...
    try {
      ...
      const tx = await writeContractAsync({
        ...
        args: [savingsPlanName, tokenAddress, tokenAmount],
      ...
      setError(
        `Failed to top up savings plan: ${error instanceof Error ? error.message : String(error)}`,
      );
      setShowTransactionModal(true);
    } finally {
      setLoading(false);
  ...
  const handleEthTopUp = async (amount: string, savingsPlanName: string) => {
    ...
    try {
      ...
      setError(
        `Failed to top up ETH savings plan: ${error instanceof Error ? error.message : String(error)}`,
      );
      setShowTransactionModal(true);
    } finally {
      setLoading(false);
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4 sm:p-0 overflow-y-auto">
      {showTransactionModal ? (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4 sm:p-0 overflow-y-auto">
          <div
            className={`${spaceGrotesk.className} bg-white rounded-3xl shadow-xl w-full max-w-md mx-auto overflow-hidden my-4 sm:my-0 max-h-[90vh] sm:max-h-none overflow-y-auto`}
          >
          ...
          <div className="p-6 relative z-10 flex flex-col max-h-full">
            ...
            <div className="mb-6">
              <div className="flex items-center bg-gradient-to-br from-white/90 to-white/80 backdrop-blur-xl rounded-xl p-4 border border-white/60 shadow-[0_10px_15px_-3px_rgba(0,0,0,0.05),0_4px_6px_-2px_rgba(0,0,0,0.03),inset_0_2px_4px_rgba(255,255,255,0.7)] hover:shadow-[0_15px_20px_-5px_rgba(129,215,180,0.1),0_8px_10px_-4px_rgba(129,215,180,0.05),inset_0_2px_4px_rgba(255,255,255,0.7)] transition-all duration-300">
                ...
                <div className="flex-1">
                  <h3 className="text-lg font-bold text-gray-800 mb-1.5 truncate max-w-[250px]">
                    {planName}
                  </h3>
                  <div className="inline-flex items-center px-3.5 py-1.5 bg-gradient-to-r from-[#81D7B4]/10 to-[#81D7B4]/5 backdrop-blur-sm rounded-full border border-[#81D7B4]/20 shadow-sm">
                    {isEth ? (
                      ...
                      <>
                        <Image
                          src={config.state.chainId === base.id ? "/base.svg" : "/celo.png"}
                          alt={getNetworkName()}
                          width={16}
                          height={16}
                          className="mr-2"
            ...
            <div className="mb-6 bg-[#81D7B4]/5 backdrop-blur-sm rounded-xl p-4 border border-[#81D7B4]/20">
              <p className="text-sm text-gray-600 italic">
                &ldquo;The habit of saving is itself an education; it fosters every virtue, teaches
                self-denial, cultivates the sense of order, trains to forethought.&rdquo;
              </p>
                        ...
                        <div className="text-xs text-green-700 space-y-0.5">
                          <div className="flex justify-between">
                            ...
                            <span className="font-medium">
                              {parseFloat(isEth ? walletBalance : tokenBalance).toFixed(4)}{" "}
                              {getTokenNameDisplay()}
                            </span>
                          </div>
                          <div className="flex justify-between">
