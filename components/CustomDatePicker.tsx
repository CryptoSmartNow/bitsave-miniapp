...
export default function CustomDatePicker({
      ...
      for (let i = 0; i < 7; i++) {
        ...
        days.push(
          <div key={day.toString()} className={`relative group`}>
            ...
            >
              <span
                className={`text-xs sm:text-sm md:text-base ${isSelected ? "animate-pulse-once" : ""}`}
              >
                {format(day, "d")}
              </span>
  ...
  return (
    <div
      className={`${spaceGrotesk.className} bg-white/20 backdrop-blur-xl rounded-xl sm:rounded-2xl border border-white/30 shadow-[0_10px_30px_-15px_rgba(0,0,0,0.25)] sm:shadow-[0_20px_50px_-15px_rgba(0,0,0,0.25)] overflow-hidden relative w-full max-w-full`}
    >
